...
// int convert_file(int p, const char *wcFilename, const char* fitqunFilename, const char* outFileName, const int maxevent = 0);
void check_files(string particles[], string exemptions[], int n_e, int nfiles);
map<string, double> map_string(string const& s);

int convert_all_files_parallel_chlep(string path, string nuancepath, const int p)
// int convert_all_files_parallel_chlep(string path, const int p)
{
  int e = 0;
  string particles[4] = {
...
//// ----------- Quick check of files ---------------
  ...
  string n_files = "0" + to_string(nfiles);
  ...
  for (int i=0; i<nfiles; i++){
    ...
    // Set input files
    string wcFile = path + "chlep02_output_wcsim/wcsim_chlep02__"+f_type+"__"+f_num+"of"+n_files+".root";
    string nuanceFile = nuancepath + "data_chlep02/extrct_chlep02__"+f_type+"__"+f_num+"of"+n_files+".txt";
    string fitqunFile = path + "chlep02_extrct_fitqun/extrct_chlep02__"+f_type+"__"+f_num+"of"+n_files+".txt";
...
//// ----------- Actual conversion loop ---------------
  ...
  for (int i=0; i<nfiles; i++){
    ...
    string f_num = "00" + to_string(i);
    f_num = f_num.substr(f_num.length()-3);
    cout << "Sim " << p << " of 4  -  File " << f_num << " of " << nfiles << endl;
    ...
    string outFile = "/lunarc/nobackup/projects/essnusb-gnn/ESSnuSB/root_events/chlep02/essnusb_chlep02_"+f_type+"_"+f_num+"of"+n_files+".root";

    // Convert
    e = convert_file(p, wcFile.c_str(), nuanceFile.c_str(), fitqunFile.c_str(), outFile.c_str());
    // e = convert_file(p, wcFile.c_str(), fitqunFile.c_str(), outFile.c_str());
    if (!(e==0)){
      cout << "Error found! Shutting down. " << endl;
      // cout << wcFile << endl << nuanceFile << endl << fitqunFile << endl;
      cout << wcFile << endl << fitqunFile << endl;
      return e;
    }
  }
  ...
  const int maxevent)
{
  ...
  // Move to fourth, no third line of fitqun and nuance files
  string f_line, n_line;
  int f_count = 0, n_count = 0;
  while (getline(fitqunFile, f_line, '\n') && f_count<1) {
    ++f_count;
  }
  while (getline(nuanceFile, n_line, '\n') && n_count<1) {
    ++n_count;
  }
...
void fill_event_nuance(ifstream *infile, int p, EventInfo *event)
{
  ...
  // cout << "call to fill_fitqun" << endl;
  // cout << "line: " << line << endl;

  map<string, double> nuance_map = map_string(line);
  // cout << endl;
  ...
  double vx=nuance_map.find("wcs_x")->second;
  double vy=nuance_map.find("wcs_y")->second;
  double vz=nuance_map.find("wcs_z")->second;
  ...
  double le=nuance_map.find("wcs_ekin")->second;;
  double ldx=nuance_map.find("wcs_dx")->second;
  double ldy=nuance_map.find("wcs_dy")->second;
  double ldz=nuance_map.find("wcs_dz")->second;
