/*
  ...
  .L PMTubeHit.cxx+
  .L EventInfo.cxx+
  .L display_event_pmts.C+
  display_event_pmts(0)
...
void display_event_pmts(Int_t event=0,
			const Char_t* file_name="troels_format.root",
			const Double_t r_d = 474, // cm
			const Double_t l_d = 1099.6) // cm
{
  gStyle->SetOptStat(0);
  ...
  for (Int_t n = 0; n < n_pmt; n++) {
    ...
    auto pmt = (PMTubeHit*)pmt_array->At(n);
